...
# 格式化输出测试报告
def generate_report(reports,save_dir,img_ct):
    ...
    img_ct[999]=0
    reports_new=[]
    for d_info in reports:
        if d_info["测例编号"]!=999:
            d_info["运行时间"]=img_ct[d_info["测例编号"]]
            reports_new.append(d_info)
    reports=reports_new
    ...
    final_result = f"\n ## 综合测试结果\n\n总得分| 功能识别精确度F | 网表识别精确度K(3s, TIMEOUTs) | 总运行时间 \n--- | --- | --- | --- | \n 略 | {final_F:.4f} | {final_K_approx:.4f}, {final_K_exact:.4f} | {K_total_ct:.4f}  \n\n"
...
def find_true(file1_path):
    ...
    parent_directory="/home/public/public/"
    # 构造“true”文件夹的绝对路径
...
def process_file(file_path):
    try:
        ...
        report={'测例编号':999,'功能识别精确度F':0,'网表识别精确度K':(999,999),'运行时间':-1}
...
# 运行所有测例的函数
def run_tests(dir_path,save_dir,img_ct):# 输入含有生成网表的文件夹路径 dir_path
    reports = []
    ...
    # 格式化并输出测试结果  
    Path("report.txt").write_text(str(reports))
    generate_report(reports,save_dir,img_ct)


def image_ct(ct_file_path):
    cost_={}
    for file_name in os.listdir(ct_file_path):
        image_name=file_name.split("_")[0]
        ct=float(file_name.split("_")[1].split(".txt")[0])
        cost_[image_name]=ct
    return cost_
...
if __name__ == '__main__':
    ...
    save_root=f"{os.getcwd()}"
    
    file_path = os.getcwd()
    dir_path=os.path.join(file_path,"generate_new")
    print(dir_path)
    ct_dir_path=os.path.join(file_path,"generate_ct")
    img_ct=image_ct(ct_dir_path)
    save_dir=save_root
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    run_tests(dir_path,save_dir,img_ct)