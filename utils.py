...
def ged(graph1, graph2,id):
    ...
    if iso_first and is_isomorphic(nx_1,nx_2,node_match,edge_match4iso):
        ...
        #ged_val = graph_edit_distance(nx_1,nx_2,node_match,edge_match,timeout = 3)
        #print(f'测例{id}识别有误。近似ged,供参考：{ged_val}')
        print('正在进一步检查，请耐心等待...')
        #cost = graph_edit_distance(nx_1,nx_2,node_match,edge_match,timeout = 900)

        cost,_= my_graph_edit_distance(nx_1,nx_2,node_match,edge_match,id,timeout =1000,)
...
def my_graph_edit_distance(G1, G2, node_match, edge_match,id, timeout):
    bestcost = []
    ...
    for _, _, cost in my_optimize_edit_paths(
        ...
        print(f"Task Name: {id}, Cost: {cost}, Time: {time_elapsed}")
